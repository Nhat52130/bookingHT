...
router.post("/create_payment_url", async function (req, res, next) {
  ...
  res.status(200).json({ code: "00", data: vnpUrl, order: save });
...
router.post("/create",async function (req, res, next)  {
  ...
  try {
    ...
    await order.save();
    res.json({ message: "Cập nhật trạng thái thanh toán thành công cho đơn hàng", orderId });
  } catch (error) {
    console.error("Lỗi khi cập nhật trạng thái thanh toán cho đơn hàng:", error);
  }
});
