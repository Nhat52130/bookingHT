    ...
    try {
        ...
        order.rooms.forEach(room => {
          room.roomNumbers.forEach(roomNumber => {
            ...
            bookedRoomDates.get(room.roomId.toString()).push({
              ...
              unavailableDates: roomNumber.unavailableDates.map(date => new Date(date).toISOString())
            });
          });
        });
      });
      ...
      const availableRooms = rooms.filter(room => {
        ...
        const allBooked = room.roomNumbers.every(roomNumber => {
          const booking = roomBookings.find(b => b.number === roomNumber.number);
          if (booking) {
      ...
      const hotelsWithRooms = hotels.map((hotel) => {
        const hotelRooms = availableRooms.filter(room => room.Hotel_id.equals(hotel._id));
        return {
          hotel,
          rooms: hotelRooms
        };
